...
Json::Value Node::getJson () const
{
    ...
    bool const hasCurrency = !isXDV (issue_.currency);
    bool const hasAccount = !isXDV (account_);
    bool const hasIssuer = !isXDV (issue_.account);
    ...
    if (!isXDV (issue_.currency))
        jvNode[jss::currency] = to_string (issue_.currency);

    if (!isXDV (issue_.account))
