    ...
    if (bValid)
    {
        ...
        else
        {
            ...
            if (!asDst)
            {
                ...
                jvDestCur.append (Json::Value ("XDV"));

                if (!saDstAmount.isNative ())
                {
                    // only XDV can be send to a non-existent account
                    bValid = false;
            ...
            {
                bool const disallowXDV (
                    asDst->peekSLE ().getFlags() & lsfDisallowXDV);

                auto usDestCurrID = accountDestCurrencies (
                        raDstAccount, crCache, !disallowXDV);
...
Json::Value PathRequest::doUpdate (RippleLineCache::ref cache, bool fast)
{
    ...
    if (sourceCurrencies.empty ())
    {
        ...
        for (auto const& c: usCurrencies)
        {
            ...
            {
                if (c.isZero ())
                    sourceCurrencies.insert ({c, xdvAccount()});
                else
    ...
    for (auto const& currIssuer: sourceCurrencies)
    {
        ...
        if (valid)
        {
            ...
            auto& sourceAccount = !isXDV (currIssuer.account)
                    ? currIssuer.account
                    : isXDV (currIssuer.currency)
                        ? xdvAccount()
                        : raSrcAccount.getAccountID ();
